...
BS.widgets[BS.W_STOLEN_ITEMS] = {
    ...
    update = function(widget)
        ...
        ttt = ttt .. BS.LC.White:Colorize(BS.BAGICON .. " " .. bagCounts.carrying)
        ...
        if (#stolen > 0) then
            ...
            for _, item in pairs(stolen) do
                ...
                stt = string.format("%s   %s%s", stt, BS.LC.Yellow:Colorize(tostring(item.sellPrice * item.count)), goldIcon)
                total = total + (item.sellPrice * item.count)
            end

            ttt = ttt .. BS.LF .. BS.LF
            ttt = ttt .. zo_strformat(GetString(_G.BARSTEWARD_TOTAL_VALUE), BS.LC.Yellow:Colorize(tostring(total)) .. goldIcon)

            ttt = ttt .. BS.LC.White:Colorize(stt)
        end
...
BS.widgets[BS.W_FENCE_TRANSACTIONS] = {
    ...
    update = function(widget)
        ...
        local noLimitColour = BS.GetVar("NoLimitColour", this) and BS.LC.White or BS.LC.Green
